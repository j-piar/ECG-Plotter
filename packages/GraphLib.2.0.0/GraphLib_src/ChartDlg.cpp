...
void CChartDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CBSHOWLEGEND, m_chkShowLegend);
	DDX_Control(pDX, IDC_CBDOUBLEBUFFER, m_chkDoubleBuffer);
	DDX_Control(pDX, IDC_CBSNAP, m_chkSnapCursor);
	DDX_Control(pDX, IDC_TOLERANCE, m_edtTolerance);	
...
/////////////////////////////////////////////////////////////////////////////
// Behandlungsroutinen f? Nachrichten CChartDlg 
...
BOOL CChartDlg::OnInitDialog() 
{
	...
	m_SpinTol.SetTheme(xtpControlThemeOfficeXP);
	m_cbCurve.SetTheme(xtpControlThemeOfficeXP);
	m_cbYAxis.SetTheme(xtpControlThemeOfficeXP);
	m_cbXAxis.SetTheme(xtpControlThemeOfficeXP);
	m_Alignment.SetTheme(xtpControlThemeOfficeXP);
//	m_InnerColor.SetTheme(xtpControlThemeOfficeXP);
//	m_CharColor.SetTheme(xtpControlThemeOfficeXP);
//	m_BkColor.SetTheme(xtpControlThemeOfficeXP);
	
	m_chkShowLegend.SetTheme(xtpControlThemeOfficeXP);
	m_chkDoubleBuffer.SetTheme(xtpControlThemeOfficeXP);
	m_chkSnapCursor.SetTheme(xtpControlThemeOfficeXP);
	m_edtTolerance.SetTheme(xtpControlThemeOfficeXP);
	...
	for (i = 0; i < m_pGraph->GetXAxisCount (); i++)
	{
		...
		cItem.Format("(%d) %s", i + 1, m_pGraph->GetPlotNameX());	// label 없을 경우를 대비해 [11/19/2013 ChoiJunHyeok]
