...
//			else
			...
			// log scale
			double minValue = pow((double)10,floor((double)log10(m_fCurMin)));
			double maxValue = pow((double)10,ceil((double)log10(m_fCurMax)));
...
double CXGraphLogAxis::GetValueForPos(int nPos)
{
	...
	else
	{
		...
		if (m_AxisKind == yAxis)
			fVal = pow((double)10,-(nPos-m_pGraph->m_clInnerRect.bottom)/(double)m_nRange *fRange + log10(m_fCurMin));
		else
			fVal = pow((double)10, (nPos-m_pGraph->m_clInnerRect.left)/(double)m_nRange * fRange + log10(m_fCurMin));
		return fVal;
	}
}
