...
void CXGraphLabel::Draw(Graphics &graphics)
{	
	if (!m_bVisible)
		return;
...
//		ps = new CPenSelector (0, 1, pDC);
	...
	if (m_nCurve != -1)
	{
		...
		int		nXPos = xAxis.GetPointForValue (&serie.m_vPointData[serie.m_nCount / 2]).x;
		int		nYPos = 0;
		double	fVal  = xAxis.GetValueForPos (nXPos);
		
		nYPos = yAxis.GetPointForValue (&serie.m_vPointData[serie.m_nCount / 2]).y;

		graphics.DrawLine(&pen,nXPos,nYPos,m_clRect.CenterPoint ().x,m_clRect.CenterPoint ().y);
...
//		pDC->LineTo(m_clRect.CenterPoint ().x, m_clRect.CenterPoint ().y);
	...
	if(!m_bTransparent)
	{
		...
		graphics.FillRectangle(&brush,RectF(m_clRect.left,m_clRect.top,m_clRect.Width(),m_clRect.Height()));
//		pDC->FillSolidRect(m_clRect,m_crColor);
	}

	
	if (m_bBorder)
		graphics.DrawRectangle(&pen,RectF(m_clRect.left,m_clRect.top,m_clRect.Width(),m_clRect.Height()));

//	pDC->SetBkColor(m_crColor);
	
//	COLORREF nOldColor = pDC->SetTextColor(m_crTextColor);
...
{	
	...
	if (!m_bTransparent)
		bs = new CBrushSelector(m_crColor, pDC);
	else
		pOldObject = pDC->SelectStockObject (NULL_BRUSH);
