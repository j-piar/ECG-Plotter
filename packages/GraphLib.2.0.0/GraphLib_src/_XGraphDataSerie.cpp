...
#ifndef _WIN32_WCE
	_tcscpy_s(serie.cLabel,260, m_cLabel);
#else
...
void CXGraphDataSerie::DrawLengend(CDCEx* pDC,CRect &rect)
{
	CPenSelector ps(m_crColor, m_nLineSize, pDC, m_nLineStyle);
	CPoint pt;
	pt.x = rect.left;
...
//--------------------------------------------------------------------------------------------------
	...
	for (int i = 0; i < m_nCount; i += nMarkerCount)
	{
		...
		point.x = xaxis.GetPointForValue(m_pData[i].fXVal) - (m_nMarkerSize / 2);
		point.y = yaxis.GetPointForValue(m_pData[i].fYVal) - (m_nMarkerSize / 2);
...
void CXGraphDataSerie::Draw(Graphics& graphics)
{
	...
	CXGraphAxis& yaxis = m_pGraph->GetYAxis(m_nYAxis);
	CXGraphAxis& xaxis = m_pGraph->GetXAxis(m_nXAxis);
	point.x = xaxis.GetPointForValue(&m_pData[0]).x;
	point.y = yaxis.GetPointForValue(&m_pData[0]).y;
...
//	CPenSelector ps(m_crColor, SelPenWidth, pDC, m_nLineStyle); 
	...
	switch(m_gtType)
	{
		case gtLine:
			{
				oldPoint.x = xaxis.GetPointForValue(m_pData[0].fXVal);
				oldPoint.y = yaxis.GetPointForValue(m_pData[0].fYVal);

				for(int i = 1; i < m_nCount; i++)
				{
					point.x = xaxis.GetPointForValue(m_pData[i].fXVal);
					point.y = yaxis.GetPointForValue(m_pData[i].fYVal);
...
void CXGraphDataSerie::Draw(CDCEx *pDC)
{
	...
	//COLOREF penColor;
	int SelPenWidth = 0;
	if (m_bSelected)	SelPenWidth = 2;
	else 				SelPenWidth = m_nLineSize;
...
//	graphics 
	...
	oldPoint = point;
	CPenSelector ps(m_crColor, SelPenWidth, pDC, m_nLineStyle); 
	if (m_gtType == gtLine)	pDC->MoveTo(point);
